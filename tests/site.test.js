...
assert(questions.length === 150, `Expected 150 questions, got ${questions.length}`);
...
assert(materials.length === 43, `Expected 43 source files, got ${materials.length}`);
...
assert(materials.filter((item) => item.href.includes("materials/presentations/נוספות/")).length === 6, "Additional teamwork and conflict decks are missing");
